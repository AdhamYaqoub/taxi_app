...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _parseRoadStatusMessages(List<dynamic> messages) async {
    ...
    for (var msg in messages) {
      try {
        ...
        if (text.contains('مسكر') || text.contains('مغلق') || text.contains('❌')) {
  ...
  Map<String, dynamic>? _extractRoadInfo(String text) {
    try {
      ...
      final roadKeywords = [
        'شارع', 'طريق', 'مدخل', 'مفرق', 'دوار', 'تقاطع',
        'street', 'road', 'entrance', 'intersection'
      ];
      ...
      if (roadName != null) {
        // تحديد المدينة
        String? city;
        if (text.contains('نابلس')) city = 'نابلس';
        else if (text.contains('رام الله')) city = 'رام الله';
        else if (text.contains('الخليل')) city = 'الخليل';
        else if (text.contains('بيت لحم')) city = 'بيت لحم';
  ...
  void _updateBlockedRoadAreas() {
    blockedRoadAreas = blockedRoads.map((road) {
      ...
      Map<String, dynamic> roadPoints = _getRoadCoordinates(road['name'], road['city']);
      ...
      if (isInBlockedCity) {
        ...
        for (var point in routePoints) {
          ...
          if (distance < (blockedArea['radius'] as double)) {
            print('تم تجنب الطريق: ${blockedArea['name']} (${blockedArea['roadType']}) في ${blockedArea['city']}');
            return true;
          }
        }
      }
  ...
  bool _isPointInCity(LatLng point, String city) {
    ...
    switch (city) {
      case 'نابلس':
        return point.latitude >= 32.21 && point.latitude <= 32.24 &&
               point.longitude >= 35.25 && point.longitude <= 35.27;
      case 'رام الله':
        return point.latitude >= 31.89 && point.latitude <= 31.92 &&
               point.longitude >= 35.19 && point.longitude <= 35.22;
      case 'الخليل':
        return point.latitude >= 31.52 && point.latitude <= 31.55 &&
               point.longitude >= 35.09 && point.longitude <= 35.12;
      case 'بيت لحم':
        return point.latitude >= 31.69 && point.latitude <= 31.72 &&
               point.longitude >= 35.19 && point.longitude <= 35.22;
      case 'سلفيت':
        return point.latitude >= 32.07 && point.latitude <= 32.10 &&
               point.longitude >= 35.15 && point.longitude <= 35.18;
      default:
        return false;
    }
  }
  ...
  double _calculateDistance(LatLng point, LatLng lineStart, LatLng lineEnd) {
    ...
    final double lineLength = distance.as(LengthUnit.Kilometer, lineStart, lineEnd);
      ...
      if (response.statusCode == 200) {
        ...
        if (data['features'].isNotEmpty) {
          ...
          for (var routeData in routesData) {
            ...
            final List<LatLng> routePoints = coords.map((coord) => LatLng(coord[1], coord[0])).toList();
                    ...
                    child: Column(
                      ...
                      children: [
                        if (blockedRoads.isEmpty)
                          Text('لا توجد طرق مغلقة حالياً')
                        else
                          ...blockedRoads.map((road) => Padding(
                                padding: const EdgeInsets.symmetric(vertical: 4),
                                child: Text('• ${road["name"]} - ${road["city"]}'),
                              )),
                      ],
                    ),
                  ),
