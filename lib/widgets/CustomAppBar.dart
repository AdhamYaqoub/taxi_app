...
class CustomAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  const CustomAppBar({Key? key, this.hiddenButtons = const []}) : super(key: key);
...
if (!hiddenButtons.contains('login'))
  PopupMenuButton<String>(
    icon: Stack(
      ...
      children: [
        Icon(Icons.account_circle, color: theme.colorScheme.onPrimary, size: 30), // أيقونة الحساب
        Positioned(
          bottom: -2,
          right: -2,
          child: Icon(Icons.add_circle, color: Colors.white, size: 15), // أيقونة + باللون الأسود
        ),
      ],
    ),
    onSelected: (value) {
    ...
    itemBuilder: (BuildContext context) => [
      ...
      PopupMenuItem(
        ...
        // زر ملف التعريف (إذا لم يكن مخفيًا)
        ...
        if (!hiddenButtons.contains('language'))
          IconButton(
            ...
            onPressed: () {
              Locale newLocale = isArabic ? const Locale('en') : const Locale('ar');
